...
CLASS lhc_zpra_mk_r_musicfestival DEFINITION INHERITING FROM cl_abap_behavior_handler
...
CLASS lhc_zpra_mk_r_musicfestival IMPLEMENTATION.
*  METHOD get_global_authorizations.
**    IF requested_authorizations-%create EQ if_abap_behv=>mk-on.
***     check create authorization
**      AUTHORITY-CHECK OBJECT 'ZPRA_MK_AO' ID 'ACTVT' FIELD '01'.
...
**      AUTHORITY-CHECK OBJECT 'ZPRA_MK_AO' ID 'ACTVT' FIELD '02'.
...
**      AUTHORITY-CHECK OBJECT 'ZPRA_MK_AO' ID 'ACTVT' FIELD '06'.
...
*  ENDMETHOD.
  ...
  METHOD get_instance_authorizations.
    ...
    READ ENTITIES OF ZPRA_Mk_R_MusicFestival IN LOCAL MODE
    ...
    LOOP AT events ASSIGNING FIELD-SYMBOL(<lfs_events>).
      IF update_requested = abap_true.
        "check authorization
*        AUTHORITY-CHECK OBJECT 'ZPRA_MK_AO'
*        ID 'ACTVT' FIELD '02'.
*        IF sy-subrc NE 0.
...
*        ENDIF.
  ...
  METHOD get_instance_features.
    DATA: music_festivals TYPE TABLE FOR READ RESULT ZPRA_MK_R_MusicFestival.
    ...
    result = VALUE #( FOR ls_event IN music_festivals
                      ( %tky = ls_event-%tky
                        %features-%assoc-_Visits = COND #(
                          WHEN ls_event-Status = zcl_pra_mk_enum_mf_status=>published
                        ...
                        %features-%delete = COND #(
                          WHEN ls_event-Status = zcl_pra_mk_enum_mf_status=>published OR
                               ls_event-Status = zcl_pra_mk_enum_mf_status=>fully_booked
                        ...
                        %features-%action-CrProj = COND #(
                          WHEN ls_event-Status = zcl_pra_mk_enum_mf_status=>published AND
  ...
  METHOD validateMandatoryValue.
    ...
    LOOP AT events REFERENCE INTO DATA(event).
      ...
      INSERT VALUE #( %tky = event->%tky %state_area = zcm_pra_mk_messages=>state_area-validate_event ) INTO TABLE reported-musicfestival.
      ...
      IF event->Title IS INITIAL OR
        ...
        INSERT VALUE #(
          %tky = event->%tky
          %state_area = zcm_pra_mk_messages=>state_area-validate_event
          "Fill in all mandatory fields to proceed.
          %msg = NEW zcm_pra_mk_messages( textid = zcm_pra_mk_messages=>event_mandatory_value_missing
  ...
  METHOD validateMaxVisitors.
    ...
    DATA: booked_visitors TYPE TABLE FOR READ RESULT ZPRA_Mk_R_MusicFestival\\Visits.
    ...
    LOOP AT events REFERENCE INTO DATA(event).
      ...
      INSERT VALUE #( %tky = event->%tky
                      %state_area = zcm_pra_mk_messages=>state_area-validate_visitors )
      ...
      IF event->MaxVisitorsNumber <= 0.
        ...
        INSERT VALUE #( %tky                       = event->%tky
                        ...
                        %msg                       = NEW zcm_pra_mk_messages( textid   = zcm_pra_mk_messages=>max_visitor_zero_negative
                                                                              severity = if_abap_behv_message=>severity-error )
      ...
      IF lines( booked_visitors ) > event->MaxVisitorsNumber.
        ...
        INSERT VALUE #( %tky                       = event->%tky
                        ...
                        %msg                       = NEW zcm_pra_mk_messages( textid   = zcm_pra_mk_messages=>max_visitors_less_than_booked
                                                                              severity = if_abap_behv_message=>severity-error )
  ...
  METHOD validateDate.
    ...
    LOOP AT events REFERENCE INTO DATA(event).
      ...
      INSERT VALUE #( %tky = event->%tky %state_area = zcm_pra_mk_messages=>state_area-validate_date ) INTO TABLE reported-musicfestival.

      IF event->EventDateTime IS NOT INITIAL AND event->EventDateTime < utclong_current( ).

        INSERT VALUE #( %tky = event->%tky ) INTO TABLE failed-musicfestival.
        INSERT VALUE #( %tky                   = event->%tky
                        %state_area            = zcm_pra_mk_messages=>state_area-validate_date
                        "Event date and time must be in the future.
                        %msg                   = NEW zcm_pra_mk_messages( textid   = zcm_pra_mk_messages=>event_datetime_invalid
                                                                          severity = if_abap_behv_message=>severity-error )
  ...
  METHOD determineStatus.
    ...
    LOOP AT events REFERENCE INTO DATA(event).
      ...
      booked_visitors = VALUE #( FOR visit IN event_visits
                                 WHERE ( ParentUuid = event->uuid
                                 AND     Status     = zcl_pra_mk_enum_visit_status=>booked )
                                 ( visit ) ).

      event->Status = COND #( WHEN event->Status IS INITIAL
                              THEN zcl_pra_mk_enum_mf_status=>in_preparation
                              WHEN event->Status = zcl_pra_mk_enum_mf_status=>fully_booked AND event->MaxVisitorsNumber <> lines( booked_visitors )
                              THEN zcl_pra_mk_enum_mf_status=>published
                              WHEN event->MaxVisitorsNumber > 0 AND event->MaxVisitorsNumber = lines( booked_visitors )
                              THEN zcl_pra_mk_enum_mf_status=>fully_booked
                              ELSE abap_off ).
    ENDLOOP.
    ...
    MODIFY ENTITIES OF ZPRA_Mk_R_MusicFestival IN LOCAL MODE
      ENTITY MusicFestival
  ...
  METHOD publish.
    ...
    LOOP AT events REFERENCE INTO DATA(event).
      ...
      IF event->Status =  zcl_pra_mk_enum_mf_status=>published OR
         event->Status =  zcl_pra_mk_enum_mf_status=>fully_booked.

        INSERT VALUE #( %tky = event->%tky ) INTO TABLE failed-musicfestival.
        INSERT VALUE #(
          %tky = event->%tky
          "The festival 'Title' has already been published.
          %msg = NEW zcm_pra_mk_messages( textid = zcm_pra_mk_messages=>event_already_published
                                          severity = if_abap_behv_message=>severity-error
                                          title = event->Title )
        ...
        WITH VALUE #( FOR mf_event IN events
                       ( %tky   = mf_event-%tky
                         Status = COND #( WHEN mf_event-FreeVisitorSeats = 0
                                          THEN zcl_pra_mk_enum_mf_status=>fully_booked
                                          ELSE zcl_pra_mk_enum_mf_status=>published ) ) ).
  ...
  METHOD createproject.
    ...
    DATA ls_project_data TYPE zcl_pra_mk_scm_ent_proj=>tys_a_enterprise_project_type.
    ...
    IF lt_event_data IS NOT INITIAL.
      ...
      DATA(lo_project) = NEW zcl_pra_mk_ent_proj_outb_integ( ).
      ...
      IF lt_proj_message IS NOT INITIAL.
        ...
        INSERT VALUE #(
          %tky = ls_mf_data-%tky
          "The festival 'Title' has already been published.
          %msg = NEW zcm_pra_mk_messages( textid = zcm_pra_mk_messages=>error_in_proj_creation
                                          severity = if_abap_behv_message=>severity-error
  ...
  METHOD generateSampleData.
    DATA create_visitors TYPE TABLE FOR CREATE ZPRA_Mk_R_Visitor.
    DATA create_music_fests TYPE TABLE FOR CREATE ZPRA_Mk_R_MusicFestival.
    DATA cba_music_fest_visits TYPE TABLE FOR CREATE ZPRA_Mk_R_MusicFestival\_Visits.
    DATA action_music_fest_publish TYPE TABLE FOR ACTION IMPORT ZPRA_Mk_R_MusicFestival~publish.
    DATA action_visits_book TYPE TABLE FOR ACTION IMPORT ZPRA_Mk_R_MusicFestival\\Visits~book.
    ...
    MODIFY ENTITY ZPRA_Mk_R_Visitor
      CREATE
      FIELDS ( Name Email )
