...
**************************************************************
"! @testing BDEF:ZPRA_MK_R_MUSICFESTIVAL
CLASS ltc_validation_methods DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CLASS-DATA:
      class_under_test     TYPE REF TO lhc_ZPRA_MK_r_musicfestival,               " the class to be tested
...
CLASS ltc_validation_methods IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds( i_for_entities = VALUE #(
        ( i_for_entity = 'ZPRA_MK_R_MUSICFESTIVAL' )
        ( i_for_entity = 'ZPRA_MK_R_VISITOR' )
        ( i_for_entity = 'ZPRA_MK_R_VISIT' ) ) ).
  ...
  METHOD validateDate.
    DATA mf_mock_data TYPE STANDARD TABLE OF ZPRA_MK_a_mf.
    TYPES: BEGIN OF ty_entity_key,
             uuid TYPE sysuuid_x16,
           END OF ty_entity_key.

    DATA: failed      TYPE RESPONSE FOR FAILED LATE ZPRA_MK_R_MusicFestival,
          reported    TYPE RESPONSE FOR REPORTED LATE ZPRA_MK_R_MusicFestival,
  ...
  METHOD validateMaxVisitors.
    ...
    DATA: mf_mock_data   TYPE STANDARD TABLE OF ZPRA_MK_a_mf,
          vstr_mock_data TYPE STANDARD TABLE OF ZPRA_MK_a_vstr,
          vst_mock_data  TYPE STANDARD TABLE OF ZPRA_MK_a_vst.
    " call the method to be tested
    TYPES: BEGIN OF ty_entity_key,
             uuid TYPE sysuuid_x16,
           END OF ty_entity_key.
...
CLASS ltc_action_methods IMPLEMENTATION.
  ...
  METHOD publish.
    ...
    DATA: result      TYPE TABLE    FOR ACTION RESULT ZPRA_MK_r_musicfestival\\MusicFestival~publish,
          mapped      TYPE RESPONSE FOR MAPPED EARLY ZPRA_MK_r_musicfestival,
          failed      TYPE RESPONSE FOR FAILED EARLY ZPRA_MK_r_musicfestival,
          reported    TYPE RESPONSE FOR REPORTED EARLY ZPRA_MK_r_musicfestival,
          entity_keys TYPE STANDARD TABLE OF ty_entity_key.
    ...
    READ ENTITY ZPRA_MK_r_musicfestival
  ...
  METHOD cancel.
    ...
    DATA: result      TYPE TABLE    FOR ACTION RESULT ZPRA_MK_r_musicfestival\\MusicFestival~cancel,
          mapped      TYPE RESPONSE FOR MAPPED EARLY ZPRA_MK_r_musicfestival,
          failed      TYPE RESPONSE FOR FAILED EARLY ZPRA_MK_r_musicfestival,
          reported    TYPE RESPONSE FOR REPORTED EARLY ZPRA_MK_r_musicfestival,
          entity_keys TYPE STANDARD TABLE OF ty_entity_key.
 ...
 TYPES: BEGIN OF ty_entity_key,
    ...
    DATA: entity_keys TYPE STANDARD TABLE OF ty_entity_key,
          lt_action  TYPE TABLE FOR ACTION IMPORT ZPRA_MK_R_MusicFestival~generateSampleData.

  lt_action = VALUE #( ( %cid = 'Root1' ) ).
  entity_keys = VALUE #( (  uuid = 'DEC190889AC21FE08191A45962D04211' ) ).


  MODIFY ENTITY ZPRA_MK_r_musicfestival
...
CLASS ltcl_determination_methods IMPLEMENTATION.
  ...
  METHOD determineStatus.
    ...
    DATA: reported    TYPE RESPONSE FOR REPORTED LATE ZPRA_MK_r_musicfestival,
          entity_keys TYPE STANDARD TABLE OF ty_entity_key.
  ...
  METHOD determineAvailableSeats.
    ...
    DATA: reported             TYPE RESPONSE FOR REPORTED  ZPRA_MK_r_musicfestival,
          failed               TYPE RESPONSE FOR FAILED  ZPRA_MK_r_musicfestival,
          mapped               TYPE RESPONSE FOR MAPPED  ZPRA_MK_r_musicfestival,
          lt_music_festival TYPE TABLE FOR READ RESULT  ZPRA_MK_r_musicfestival,
          entity_keys          TYPE STANDARD TABLE OF ty_entity_key.

    MODIFY ENTITIES OF ZPRA_MK_r_musicfestival
...
CLASS ltc_authorization_methods DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    TYPES s_reported_early TYPE RESPONSE FOR REPORTED EARLY ZPRA_MK_R_MusicFestival.
    TYPES s_failed_early TYPE RESPONSE FOR FAILED EARLY ZPRA_MK_R_MusicFestival.
...
*    DATA ls_requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZPRA_MK_R_MusicFestival\\MusicFestival.
*    DATA lt_result TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZPRA_MK_R_MusicFestival\\MusicFestival.
*
...
*    DATA lt_entity_keys TYPE TABLE FOR AUTHORIZATION KEY ZPRA_MK_R_MusicFestival\\MusicFestival.
*    DATA ls_requested_authorizations TYPE STRUCTURE FOR AUTHORIZATION REQUEST ZPRA_MK_R_MusicFestival\\MusicFestival.
*    DATA lt_result TYPE TABLE FOR AUTHORIZATION RESULT ZPRA_MK_R_MusicFestival\\MusicFestival.
